...
class MapOfManuallyChunkedIndexesBuilderTest {
    ...
    void addThrows() {
        //given
        MapOfManuallyChunkedIndexesBuilder<TestTags> builder =
                new MapOfManuallyChunkedIndexesBuilder<>(getDefaultSizes(asList(TestTags.values())));
        //execute & assert
    ...
    void add() {
        ...
        Map<String, int[]> unpackedIndex1 = built.get(TestTags.TAG1).chunks()
                .collect(toMap(
                        chunk -> chunk.getChunkInfo().getLabel(),
                        chunk -> chunk.ids().toArray()));
        Map<String, int[]> unpackedIndex2 = built.get(TestTags.TAG2).chunks()
                .collect(toMap(
                        chunk -> chunk.getChunkInfo().getLabel(),
                        chunk -> chunk.ids().toArray()));

        //assert
        assertThat(unpackedIndex1.entrySet().size()).isEqualTo(1);
        assertThat(unpackedIndex1.get("chunk1"))
                .containsExactly(10);
        assertThat(unpackedIndex2.entrySet().size()).isEqualTo(2);
        assertThat(unpackedIndex2.get("chunk1"))
                .containsExactly(10);
        assertThat(unpackedIndex2.get("chunk2"))
