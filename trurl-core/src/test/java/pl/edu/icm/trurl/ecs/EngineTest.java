...
class EngineTest {
    ...
    void before() {
        ...
        lenient().when(storeFactory.create()).thenReturn(store);
    ...
    void construct() {
        // execute
        new Engine(storeFactory, mapperSet);
    ...
    void execute() {
        // given
        Engine engine = new Engine(store, mapperSet);
    ...
    void onUnderlyingDataChanged() {
        ...
        // execute
        engine.onUnderlyingDataChanged(0, 100);

        verify(mapperA).create();
        verify(mapperA, times(50)).load(any(), any(), anyInt());
        verify(mapperListenersA, times(50)).fireSavingComponent(eq(component), anyInt());
