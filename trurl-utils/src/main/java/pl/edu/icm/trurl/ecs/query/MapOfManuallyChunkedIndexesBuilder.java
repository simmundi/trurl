...
public class MapOfManuallyChunkedIndexesBuilder<T> implements RawQuery.Result<T> {
    private final Map<T, ManuallyChunkedRawIndexBuilder<?>> orBuilders;

    public MapOfManuallyChunkedIndexesBuilder(Map<T, Integer> tagClassifiersWithInitialSizes) {
        ...
        orBuilders = Collections.unmodifiableMap(map);
    }
    ...
    public void add(int entityId, String tag, T tagClassifier) {
        orBuilders.get(tagClassifier).add(entityId, tag);
    }

    public Map<T, RandomAccessIndex> build() {
        return orBuilders.entrySet().stream().collect(toMap(Map.Entry::getKey, entry -> entry.getValue().build()));
    }
}
