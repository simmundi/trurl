...
public class GetChildDaosFeature implements Feature {
    ...
    public Stream<MethodSpec> methods() {
        return Stream.of(overrideGetChildDaos(beanMetadata));
    }

    private MethodSpec overrideGetChildDaos(BeanMetadata beanMetadata) {
        List<ComponentProperty> childDaos = beanMetadata
        ...
        return MethodSpec.methodBuilder("getChildDaos")
                ...
                .addCode(childDaos.isEmpty() ? emptyList() : listDaos(childDaos))
                .build();
    }

    private CodeBlock listDaos(List<ComponentProperty> daoProperties) {
        return CodeBlock.builder()
                .addStatement("return $T.asList($L)", CommonTypes.ARRAYS, daoProperties.stream()
                        .map(p -> p.fieldName)
